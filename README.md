...
게임 규칙은 같지만 화면 크기에 맞춰 사과 밀도와 기기 최고 기록을 분리합니다.

| 환경 | 게임판 | HUD | 시작 화면 |
| --- | --- | --- | --- |
| 모바일 | `6열 × 12행` | 상단 가로 HUD | 세로 단일 흐름 |
| 데스크톱 | `14열 × 7행` | 상단 가로 HUD | 독립형 2단 카드 |
...
- 시간·점수·효과음을 게임판 위 가로 HUD에 배치
- 화면 너비와 높이를 함께 계산하되 데스크톱 게임판은 약 1,100px 안에서 가독성 있게 표시
- 데스크톱 로비는 점수판과 게임 프레임을 숨긴 독립형 2단 카드로 구성
- 모바일·데스크톱 최고 기록을 별도 저장
...
- 2.8초 안에 연속 정답을 맞히면 콤보 표시
- 사과 3개 이상을 한 번에 제거하면 `BIG POP` 표시
- 콤보와 `BIG POP`은 보너스 점수 없이 결과 통계에만 기록
